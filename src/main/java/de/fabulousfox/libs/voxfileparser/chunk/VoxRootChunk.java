package de.fabulousfox.libs.voxfileparser.chunk;

import de.fabulousfox.libs.voxfileparser.GridPoint3;
import de.fabulousfox.libs.voxfileparser.VoxModelBlueprint;
import de.fabulousfox.libs.voxfileparser.VoxModelInstance;
...
public final class VoxRootChunk extends VoxChunk {
	...
	public static VoxRootChunk read(InputStream stream, InputStream childrenStream) throws IOException {
		...
		VoxChunk first = readChunk(childrenStream);
		...
		while (childrenStream.available() > 0) {
			...
			if (chunk1 instanceof VoxSizeChunk) {
				VoxChunk chunk2 = readChunk(childrenStream, ChunkFactory.XYZI);
